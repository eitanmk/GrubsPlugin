...
public class GrubsItemCommand implements GrubsCommandHandler {
	private final static Logger log = Logger.getLogger("Minecraft");
	
	public static HashMap<String,ItemStack[]> itemKitPresets = new HashMap<String,ItemStack[]>();
	public static HashMap<String,ItemStack[]> itemKitArmorPresets = new HashMap<String,ItemStack[]>();
	public static String itemKitMainDirectory = "plugins/ItemKitPresets";
	public static File ItemKitPresetFile = new File(itemKitMainDirectory + File.separator + "presets.dat");
	static Properties itemKitProperties = new Properties(); 
	
	private static GrubsItemCommand instance = null;
	private GrubsItemCommand() { }
	public static GrubsCommandHandler getInstance() {
		if (instance == null) {
			instance = new GrubsItemCommand();
		}
	...
	public static void enable(Server server) {
		log.info("[GrubsPlugin]: Initializing Item Kit functionality.");
		File mainDir = new File(itemKitMainDirectory);
		if (!mainDir.exists()) {
			log.info("[GrubsPlugin]: Item Kit save directory doesn't exist. Creating.");
			mainDir.mkdir();
		}
		
		if(!ItemKitPresetFile.exists()){
			log.info("[GrubsPlugin]: Item Kit preset file doesn't exist. Creating.");
			try {
				ItemKitPresetFile.createNewFile();
			} 
			catch (IOException ex) {
				log.info("[GrubsPlugin]: Error creating Item Kit preset file!");
				ex.printStackTrace();
			}
		}
		
		log.info("[GrubsPlugin]: Loading Item Kit presets.");
		loadItemKits();
		log.info("[GrubsPlugin]: Loaded " + GrubsItemCommand.itemKitPresets.size() + " presets.");
		
	}
	
	public static void disable() {
		log.info("[GrubsPlugin]: Saving Item Kit presets.");
		saveItemKits();
	}
	
	static void loadItemKits() {
		try {
	...
	static void saveItemKits() {
		...
		log.info("[GrubsPlugin]: Writing Item Kit presets file.");
		try {
			...
			log.info("[GrubsPlugin]: Error writing Item Kit presets file!");
			ex.printStackTrace();
		}
	}
